...
Pending upstream migration to ``oold.utils.mappings`` in `OO-LD/oold-python
<https://github.com/OO-LD/oold-python>`_: as of oold 0.20.0 the library knows ``x-oold-context``
only for validation (``validation.context_resolution.promoted_terms`` marks a term as mapped
without ever choosing a synonym, per ``OOLD-EXT-8f62``), while the *selection* itself lives
here. Ported from the generators in `OO-LD/oold-reference-schemas
