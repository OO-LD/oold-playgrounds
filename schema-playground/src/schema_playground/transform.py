...
with one mapping set promoted (see :mod:`schema_playground.mappings`). Reading that graph back under
...
Pending upstream migration to ``oold.utils.transform`` in `OO-LD/oold-python
<https://github.com/OO-LD/oold-python>`_, where (as of 0.20.0) that module still implements an
earlier notation that encoded synonyms as ``"name*"`` sibling keys in the context. That
notation predates the specification; ``x-oold-context`` (OO-LD 1.0.0-rc.2) expresses the same
relation with a mapping predicate and a set identifier, and is what the meta-schema validates.
Migrating this module upstream replaces that one, which is a breaking change there.
"""
...
from schema_playground.mappings import declared_context, promote, synonym_entries
...
def to_rdf(
    ...
    ``schemas`` is the inheritance chain, base first (see :func:`schema_playground.mappings.chain`).
