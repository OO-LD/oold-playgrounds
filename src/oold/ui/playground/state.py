...
from oold.ui.playground.graph import graph_data
from oold.utils.mappings import chain, mapping_sets
from oold.utils.transform import JSON_LD, NQUADS, TURTLE, from_rdf, to_rdf
...
def _fetch_ref(ref: str) -> dict[str, Any] | None:
    """Resolve an ``allOf`` ``$ref`` that names a remote schema.

    A relative reference has no base here - the schema was typed in, not loaded from a
    directory - so only absolute URLs can be followed. Anything else is skipped, leaving the
    chain shorter rather than failing the whole render.
    """
    if not cfg.looks_like_url(ref):
        return None
    try:
        return json.loads(cfg.fetch(ref))
    except Exception as exc:
        logger.warning("could not resolve $ref %s: %s", ref, exc)
        return None
...
def validate(document: Any, kind: str, schema: Any = None) -> str:
    """Run the real validator over an in-memory document and report what failed.

    The pipeline reads from disk (it inspects sibling schemas for reference cycles), so the
    documents are written to a temporary directory. That works unchanged in a browser, whose
    filesystem is in memory anyway.
    """
    ...
    try:
        with tempfile.TemporaryDirectory() as directory:
            ...
            if kind == "schema":
                ...
                # The file must be named as the schema's own $id, or a "Thing.schema.json"
                # style reference inside its @context resolves to nothing and the round-trip
                # check reports an empty graph rather than the real problem.
                ...
                payload = {k: v for k, v in document.items() if k != "$schema"}
...
class PlaygroundState(param.Parameterized):
    ...
    def _chain(self, schema: Any) -> list[dict[str, Any]]:
        return chain(schema, _fetch_ref) if isinstance(schema, dict) else []
    ...
    def recompute(self) -> None:
        ...
        source_chain = self._chain(source_schema)
        target_chain = self._chain(target_schema)
        ...
        if source_chain and isinstance(source_instance, dict) and not instance_error:
            try:
                ...
                source_rdf = to_rdf(
                    source_instance, source_chain, set_id=set_id, format=self.rdf_format
                )
            except Exception as exc:
        ...
        with param.parameterized.batch_call_watchers(self):
            ...
            self.source_schema_error = schema_error or validate(source_schema, "schema")
            self.source_instance_error = instance_error or validate(
                source_instance, "instance", source_schema
            )
            self.target_schema_error = target_error or validate(target_schema, "schema")
...
def _reserialize(nquads: str, format: str, schemas: list[dict[str, Any]]) -> str:
    ...
    if format == NQUADS:
        return nquads
    if format == JSON_LD:
        from oold.utils.mappings import declared_context
        from pyld import jsonld
